...
function GeneratingOverlay({
  ...
  const phaseLabels: Record<string, string> = {
    ...
    ui_components: 'Создание UI...',
    styling: 'Стилизация...',
    logic: 'Логика приложения...',
    integration: 'Интеграция...',
    validation: 'Проверка...',
    refinement: 'Улучшения...',
  };

  const displayPhase = phase ? (phaseLabels[phase] || phase) : `Итерация ${iteration}/${maxIterations}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Status text */}
        <div>
          <p className="font-medium text-foreground">Применяю изменения...</p>
        ...
        {filesCount > 0 && (
          ...
          >
            {filesCount} файл{filesCount === 1 ? '' : filesCount < 5 ? 'а' : 'ов'} готово
          </motion.p>
        )}
...
export const PreviewPanel = memo<PreviewPanelProps>(function PreviewPanel({
  ...
  const useCanvasHostPreview =
    String(import.meta.env.VITE_USE_CANVAS_HOST_PREVIEW || "").toLowerCase() === "true";
  const primaryPreviewUrl = useCanvasHostPreview
    ? previewSession.canvasUrl || previewSession.previewUrl
    : previewSession.previewUrl || previewSession.canvasUrl;
            ...
            {hasRealContent ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center text-center p-4">
                ...
                <p className="text-[10px] text-muted-foreground/60 mt-1">
                  Итерация {orchestratorIteration}/{orchestratorMaxIterations}
                </p>
              </div>
            ) : !hasContent ? (
    ...
    return (
      <main data-tour="preview" className={cn("flex-1 h-full min-h-0 bg-muted/20 overflow-hidden relative", className)}>
        <GeneratingAnimation 
          iteration={orchestratorIteration} 
          maxIterations={orchestratorMaxIterations}
          filesCount={reactProject?.files.length || 0}
        />
      </main>
    );
  }
