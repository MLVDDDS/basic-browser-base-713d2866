...
import type { ProjectCodePanelProps } from './ProjectCodePanel';
...
export interface BuilderHeaderProps {
  ...
  historyMode: boolean;
  historyTab: 'timeline' | 'changes';
  historyCount: number;
  onEnterHistoryMode: () => void;
  onExitHistoryMode: () => void;
  onHistoryTabChange: (tab: 'timeline' | 'changes') => void;
...
const ProjectCodePanel = lazy(() =>
  import('./ProjectCodePanel').then((module) => ({ default: module.ProjectCodePanel }))
);

function ProjectCodePanelFallback() {
  return (
    <Button variant="ghost" size="icon" className="h-9 w-9" disabled aria-label="Загрузка кода">
      <Code2 className="w-4 h-4" />
    </Button>
  );
}

function LazyProjectCodePanel(props: ProjectCodePanelProps) {
  return (
    <Suspense fallback={<ProjectCodePanelFallback />}>
...
export function BuilderHeader({
  ...
  return (
    ...
    <header className="h-16 border-b border-border/70 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      ...
      {historyMode ? (
        <div className="hidden md:flex items-center gap-1 rounded-lg border border-border/70 bg-muted/25 px-1 py-1">
          <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={onExitHistoryMode}>
            Back to latest
          </Button>
          <div className="h-5 w-px bg-border/60" />
          <Button
            variant={historyTab === 'timeline' ? 'secondary' : 'ghost'}
            size="sm"
            className="h-8 text-xs"
            onClick={() => onHistoryTabChange('timeline')}
          >
            Timeline
          </Button>
          <Button
            variant={historyTab === 'changes' ? 'secondary' : 'ghost'}
            size="sm"
            className="h-8 text-xs"
            onClick={() => onHistoryTabChange('changes')}
          >
            Changes
          </Button>
        </div>
        ...
        <Button
          variant="ghost"
          size="sm"
          className="hidden md:inline-flex h-8 gap-2 text-xs"
      ...
      <div className="flex md:hidden items-center gap-1">
        {historyMode ? (
          <>
            <Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={onExitHistoryMode}>
              Latest
            </Button>
            <Button
              variant={historyTab === 'timeline' ? 'secondary' : 'ghost'}
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onEnterHistoryMode}>
