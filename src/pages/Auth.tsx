...
declare global {
  interface Window {
    ...
    __lyubakodTelegramAuth?: (user: TelegramAuthUser) => void;
  }
}
...
const Auth = ({ mode }: AuthProps) => {
  ...
  const { signInWithEmail, signUpWithEmail, signInWithGoogle, signInWithTelegram } = useAuth();
  ...
  const telegramBotUsername = String(import.meta.env.VITE_TELEGRAM_BOT_USERNAME || '').trim();
  ...
  const getReadableError = (error: Error): string => {
    ...
    if (msg.includes('telegram_not_configured')) {
      return 'Telegram вход пока не настроен на сервере';
    }
    if (msg.includes('telegram_hash_invalid') || msg.includes('telegram_hash_required')) {
      return 'Не удалось подтвердить вход через Telegram. Попробуйте еще раз';
    }
    if (msg.includes('telegram_auth_expired')) {
      return 'Сессия Telegram устарела. Запустите вход еще раз';
    }
    if (msg.includes('telegram_account_conflict')) {
      return 'Этот Telegram-аккаунт уже привязан к другому профилю';
    }
  ...
  const handleTelegramAuth = useCallback(
    async (telegramUser: TelegramAuthUser) => {
      ...
      const { error } = await signInWithTelegram({
        ...
        photo_url: String(telegramUser?.photo_url || ''),
    ...
    [navigateAfterAuth, signInWithTelegram]
  );
  ...
  const handleTelegramLogin = () => {
    if (!telegramBotUsername) {
      setAuthError('Telegram Login Widget не настроен. Нужен VITE_TELEGRAM_BOT_USERNAME');
      return;
    }
    setAuthError(null);
  ...
  useEffect(() => {
    if (!telegramWidgetVisible || !telegramBotUsername || !telegramWidgetRef.current) {
      return;
    }
    ...
    container.innerHTML = '';
    window.__lyubakodTelegramAuth = (user: TelegramAuthUser) => {
      if (cancelled) return;
    ...
    script.setAttribute('data-onauth', '__lyubakodTelegramAuth(user)');
    script.onerror = () => {
      if (cancelled) return;
      setAuthError('Не удалось загрузить Telegram Login Widget');
    };
    ...
    return () => {
      cancelled = true;
      container.innerHTML = '';
      delete window.__lyubakodTelegramAuth;
    };
  }, [handleTelegramAuth, telegramBotUsername, telegramWidgetVisible]);
        ...
        >
          ...
          >
            ...
            <motion.div variants={itemVariants} className="space-y-3">
              ...
              >
                <TelegramIcon />
                {telegramSubmitting ? 'Подтверждаем вход...' : 'Войти через Telegram'}
              </Button>
              {telegramWidgetVisible && (
                <div className="rounded-xl border border-border bg-muted/30 px-4 py-4">
                  <p className="mb-3 text-xs text-muted-foreground text-center">
                    Продолжи вход через официальный Telegram Login Widget
                  </p>
