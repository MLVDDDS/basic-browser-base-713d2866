...
/**
 * Hook for preprocessing user prompts using fast-tier AI with AUTO-CONTEXT
 * 
 * Now context-aware:
...
export function usePromptPreprocessor(): UsePromptPreprocessorReturn {
  ...
  const preprocess = useCallback(async (
    ...
    try {
      // For existing projects and file-based tasks, always prefer server-side semantic preprocessing.
      // Local heuristics stay only as an emergency fallback for brand new ultra-short prompts.
      const wordCount = prompt.trim().split(/\s+/).length;
      const shouldUseLocalFallbackOnly =
        !builderContext.hasProject &&
        !hasAttachedFiles &&
        wordCount <= 3;
...
function inferQuickAction(
  ...
  const critiqueOrImprove =
    /(шаблон|дизайн|визуал|стиль|hero|ui|ux|layout|верстк|типограф|отступ|лендинг|сайт|доработ|улучш|передел|красив|сильн|better|improve|design|polish)/i.test(text);
  const layoutFix =
    /(верстк|layout|типограф|отступ|съехал|съехало|выровн|почин|исправ)/i.test(text);

  if (hasProject && layoutFix) {
    return {
      intent: context?.conversationState?.activeIssue || 'fix_current_layout',
      actionType: 'fix',
      target: 'project',
    };
  }

  if (hasProject && critiqueOrImprove) {
    return {
      intent: context?.conversationState?.activeIssue || 'improve_current_project',
      actionType: 'improve',
      target: 'project',
    };
  }

  if (hasProject && /(продолж|дальше|ещ[её]|continue|keep going|go on)/i.test(text)) {
    return {
      intent: 'continue_current_project',
      actionType: 'edit',
      target: 'project',
    };
  }
