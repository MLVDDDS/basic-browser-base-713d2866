...
import {
  ...
  fetchPreviewSessionStatus,
  pushPreviewSessionFiles,
  restartPreviewSession as restartPreviewSessionRequest,
  stopPreviewSession as stopPreviewSessionRequest,
} from "@/features/builder/api/preview-sessions-api";
...
const PREVIEW_SESSION_STORAGE_PREFIX = "builder.previewSession";

function getStoredPreviewSessionKey(projectId: string): string {
  return `${PREVIEW_SESSION_STORAGE_PREFIX}:${projectId}`;
}

function readStoredPreviewSessionId(projectId?: string): string | null {
  if (!projectId || typeof window === "undefined") return null;
  try {
    const stored = window.localStorage.getItem(getStoredPreviewSessionKey(projectId));
...
function writeStoredPreviewSessionId(projectId: string, sessionId: string | null) {
  if (!projectId || typeof window === "undefined") return;
  try {
    const key = getStoredPreviewSessionKey(projectId);
    if (sessionId) {
...
export function usePreviewSession({
  ...
  useEffect(() => {
    if (!projectId) return;
    writeStoredPreviewSessionId(projectId, sessionId);
  }, [projectId, sessionId]);
  ...
  const stopSession = useCallback(
    async (targetSessionId?: string | null) => {
      ...
      try {
        await stopPreviewSessionRequest(activeSessionId);
      } catch (err) {
        ...
        if (projectIdRef.current) {
          writeStoredPreviewSessionId(projectIdRef.current, null);
        }
      }
    },
    [apiEnabled]
  ...
  useEffect(() => {
    ...
    if (previousProjectId && previousSessionId) {
      void stopSession(previousSessionId);
    }
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      const response = await createPreviewSession(projectId, versionId);
      let session = response.session;
      if (shouldRestartStaleActiveSession(session)) {
        try {
          const restarted = await restartPreviewSessionRequest(session.id);
      ...
      if (isUnauthorizedApiError(err)) {
        setAuthBlocked(true);
        setError("Сессия истекла. Войди заново, чтобы запустить preview.");
        return;
      }
  ...
  const restartSession = useCallback(async () => {
    ...
    if (activeSessionId) {
      try {
        const response = await restartPreviewSessionRequest(activeSessionId);
  ...
  const refreshStatus = useCallback(async () => {
    const activeSessionId = sessionIdRef.current;
    if (!apiEnabled || !activeSessionId || authBlocked) return;

    try {
      const response = await fetchPreviewSessionStatus(activeSessionId);
      ...
      if (isUnauthorizedApiError(err)) {
        setAuthBlocked(true);
        setError("Сессия истекла. Войди заново, чтобы продолжить работу с preview.");
        return;
      }
      const statusCode =
  ...
  }, [apiEnabled, authBlocked, restartSession, startSession]);
  ...
  const pushFiles = useCallback(
    async (files: Record<string, string | null>) => {
      ...
      try {
        await pushPreviewSessionFiles(activeSessionId, files);
      } catch (err) {
        if (isUnauthorizedApiError(err)) {
          setAuthBlocked(true);
          setError("Сессия истекла. Войди заново, чтобы синхронизировать preview.");
          return;
        }
  ...
  useEffect(() => {
    ...
    const attemptKey = projectId;
    const restoredSessionId = readStoredPreviewSessionId(projectId);
    if (!sessionIdRef.current && restoredSessionId) {
      startAttemptKeyRef.current = attemptKey;
      setSessionId(restoredSessionId);
      setStatus((current) => current || "queued");
      setRuntimeState((current) => current || "building");
      setError(null);
      return () => {
        if (pollRef.current) {
  ...
  }, [apiEnabled, enabled, projectId, versionId, authBlocked, startSession]);
  ...
  useEffect(() => {
    if (!sessionId || !apiEnabled || authBlocked) return;
    if (pollRef.current) {
      window.clearInterval(pollRef.current);
    }
    void refreshStatus();
  ...
  }, [apiEnabled, authBlocked, refreshStatus, sessionId]);
  ...
  return useMemo(
    () => ({
      ...
      authBlocked,
      isStarting,
      canRestart: Boolean(apiEnabled && enabled && projectId && !authBlocked),
      startSession,
      stopSession,
      restartSession,
      pushFiles,
