...
function normalizeApiUser(raw: unknown): ApiAuthUser | null {
  ...
  const normalizedEmail =
    provider === "telegram" && typeof email === "string" && email.endsWith("@telegram.local")
      ? null
      : email;
  return {
    id,
    ...
    provider,
    telegram_username:
      typeof source.telegram_username === "string" ? source.telegram_username : null,
...
export async function apiTelegramLogin(payload: TelegramLoginPayload) {
  return requestApiAuth("/auth/telegram", payload as unknown as Record<string, unknown>);
}

export async function apiTelegramMiniAppLogin(initData: string) {
  return requestApiAuth("/auth/telegram/tma", {
    initData,
  });
}
